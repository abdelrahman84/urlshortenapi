# urlshortenapi
This is a simple app which shorten URL to a unique 8 alphanumeric characters.

Backend is built with nodejs, mongodb, and front end is built with Vuejs. The full app is dockerized.

To install, you can either use the docker file, by running the command 'docker-compose up' from the root directly, or by goind to each directory sperartely:

1- in front end, run 'npm install', then 'npm run serve.'

2- for backend, run 'npm install', then 'npm start'

A short video which shows the app, can be found below:

https://drive.google.com/file/d/17c17HZlp-6M70cUDtBtiPHJW_K_K1cqN/view?usp=sharing
